...
- **The snow remembers.** Whatever presses on it leaves a print: the fox's paw prints and the furrow it wades, the wings Chimin's arms and legs sweep, the cursor's touch. Prints are real hollows (the ground and the letters sink, the far wall lit, the near wall in shadow) and fill back in over about a minute as the snow keeps falling.
- **Paper, not plastic.** The snow and the letters carry handmade-paper fibre and speckle, and their tones sit in a few flat patches, so the field is in the same hand as Chimin's pieces; the light effects (glitter, glints, rim, aurora) stay.
...
## Inside (three stops)
The same paper hand, walked through. You come in and the camera swings left to the coat hooks; scroll (or the arrow keys, or the row of names at the bottom right) walks you on to the bed, then to the window. Each stop is close on what there is to touch, and each thing opens over the room.
- **The coats.** A white puffer and a brown leather shoulder bag on hooks by the door. Hover the bag and the laptop rises out of it; click and it comes fully out, lies open in the middle of the view, and the technical projects appear. The enamel pins on the bag are the design projects: hover and a pin swells and glistens; click and it floats up large on the left while a panel opens on the right with that project.
- **The bed.** The fox asleep on the quilt. On the bedside table a stack of magazines fans out on hover into the essays; click one and it opens as a magazine in the middle of the screen, the cover first, then spreads that turn when you click a page. A record player on the bench plays a curated list; it is quieter from the other stops and full here.
- **The window.** The round window whole in the frame, with the letter snow and the sky outside (the same field and time of day). On the chest below: the camera (photographs), the vase, the paints (about), and postcards scattered loose. Touch the postcards and they spread out into every place; tap one and it turns and zooms, and its back is empty paper (no lines) to paste the stamps, sketches and stickers onto, kept in the browser.
